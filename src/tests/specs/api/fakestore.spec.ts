...
import { apiBaseURL, apiUsername, apiPassword, apiEmail} from "../../../utils/localVariables.ts";
...
test.describe('FakeStore API tests', () => {
    ...
    test("get users to do the login tests", async ({ request }) => {
        ...
        console.log('User Response Body:', newUserResponse);
    });
    ...
    test("get products to do the test with  products", async ({ request }) => {
        ...
        const prodsResponse = await prods.json();
        console.log('Products Response Body:', prodsResponse);
    });
    ...
    test("get product and validate its content", async ({ request }) => {
        ...
        expect(newUserResponse).toHaveProperty('title', 'John Hardy Women\'s Legends Naga Gold & Silver Dragon Station Chain Bracelet');
